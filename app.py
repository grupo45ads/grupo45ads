...
A aplicação apresenta métricas gerais, visualizações gráficas e filtros interativos com base nos dados das partidas da competição.
""")
...
try:
    ...
    st.error("Arquivo CSV não encontrado. Verifique se o arquivo está na pasta BaseDados com o nome Fifa_world_cup_matches.csv.")
    st.stop()
...
data_col = encontrar_coluna(possiveis_data)

# =========================
# Validação das colunas
# =========================

if not (time_1_col and time_2_col and gols_1_col and gols_2_col):
    ...
    st.write("Colunas disponíveis na base:")
    st.write(df.columns.tolist())
    st.dataframe(df.head())
    st.stop()
...
st.caption(
    f"A seleção com mais vitórias possui {quantidade_vitorias} vitória(s). "
    f"A seleção com mais gols marcou {quantidade_gols_selecao} gol(s)."
)
...
# =========================
# Gráfico 4: Distribuição de resultados
# =========================

st.subheader("📈 Distribuição dos resultados")

resultados = df_filtrado["vencedor"].value_counts().reset_index()
...
fig_resultados = px.pie(
    ...
    names="Resultado",
    values="Quantidade",
    title="Distribuição de vitórias e empates"
)
...
Este dashboard foi desenvolvido para apresentar uma análise visual dos dados da Copa do Mundo FIFA 2022.
A partir dele, é possível observar o número total de partidas, quantidade de gols, média de gols por jogo,
seleções com mais vitórias, seleções com mais gols e a distribuição das partidas por fase da competição.
""")
